...
class SchemaDefinition(env: Env) {
  ...
  lazy val  NotificationStatusType: InterfaceType[(DataStore, DaikokuActionContext[JsValue]), NotificationStatus] = InterfaceType(
    ...
    () => fields[(DataStore, DaikokuActionContext[JsValue]), NotificationStatus](
      Field("status", StringType, resolve = _.value.toString) // TODO - can't generate interface without fields
    )
  )
  ...
  lazy val  NotificationStatusPendingType = new PossibleObject(deriveObjectType[(DataStore, DaikokuActionContext[JsValue]), NotificationStatus.Pending](
    ...
    AddFields(
      Field("type", StringType, resolve = _.value.toString) // TODO - can't generate interface without fields
    )
  ))
  ...
  lazy val  NotificationActionType: InterfaceType[(DataStore, DaikokuActionContext[JsValue]), NotificationAction] = InterfaceType(
    ...
    () => fields[(DataStore, DaikokuActionContext[JsValue]), NotificationAction](
      Field("type", StringType, resolve = _.value.getClass.getName) // TODO - can't generate interface without fields
    )
  )
  ...
  lazy val  NotificationType: ObjectType[(DataStore, DaikokuActionContext[JsValue]), Notification] = deriveObjectType[(DataStore, DaikokuActionContext[JsValue]), Notification](
    ...
    ReplaceField("notificationType", Field("notificationType", StringType, resolve = _.value.notificationType.value)),
    ...
    (
      Schema(ObjectType("Query",
        () => fields[(DataStore, DaikokuActionContext[JsValue]), Unit](
          ...
          ++ teamQueryFields()
          ++ apiQueryFields()
          ++ new NotificationsQueries(this).queries():_*)
      )),
